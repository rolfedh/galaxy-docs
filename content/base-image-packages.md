...
XXX make images and repo public
...
Next, it installs the latest version of npm. It does this because the version of Node that ships with older versions of Meteor used an older version of npm which did not support some modern npm features.
...
Otherwise, the script runs `node $NODE_OPTIONS main.js` inside the `/app/bundle` directory. You can set `$NODE_OPTIONS` to a flag or space-separated series of flags if you need fine-grained control over how Node runs your server, such as setting [garbage collection flags](/scaling.html#garbage-collection).
